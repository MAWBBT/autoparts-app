...
def _persist_store():
    """
    Хранилище состояния между перезагрузками страницы (F5) в рамках процесса Streamlit.
    Важно: это НЕ переживает перезапуск сервера и при многопользовательском доступе будет общим.
    """
    return {}
...
if "main_data" not in st.session_state and isinstance(persist.get("main_data"), pd.DataFrame):
    st.session_state.main_data = persist["main_data"].copy(deep=True)
